                ...
                <div class="kt-portlet">
                    <div class="kt-portlet__body">
                        <div class="kt-section">
                            <h3 class="kt-section__title">FACTURE PROFORMAT</h3>
                            <div class="kt-section__info">Utilise les paramètres suivants pour faire une proformat</div>
                                    
                            <div class="kt-widget__toolbar">
                                <button class="btn btn-icon btn-circle btn-label-facebook fpbtn" id="fpbtn">
                                    <i class="fa fa-eye"></i> 
                                </button> Historique
                            </div>
                                            ...
                                            <div class="row">
                                                <div class="col-lg-4 ml-lg-auto">
                                                    ...
                                                    <select class="form-control depot" id="depot" name="depot">
                                                        <?php if(!empty($depot)){?> 
                                                            <?php echo '<option value='.$depot['mat_depot'].'>'.$depot['nom_depot'].'</option>'; ?> 
                                                        <?php }else if(!empty($depots)){ ?>
                                                ...
                                                <div class="col-lg-4 ml-lg-auto">
                                                    <span class="form-text text-muted">Delais Règlement jour(s)</span>
                                                    <input type="text" name="delais" id="delais" class="form-control delais" value="45" placeholder="quel est le delais?">
                                                    <span id="delais_error" class="text-danger"></span>
                                                </div>
                                                <div class="col-lg-4 ml-lg-auto">
                                        ...
                                        <div class="kt-form__actions">
                                            <div class="row">
                                                <div class="col-lg-5 ml-lg-auto">
                                                    ...
                                                    <select class="form-control kt-select2 document" id="kt_select2_3" name="document">
    ...
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <input type="hidden" name="delai" id="delai" class="delai">
...
<!-- liste des factures proformat debut -->
<div class="modal fade" tabindex="-1" role="dialog" id="fp">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Liste des factures proformat</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <input type="search" name="recherche" id="recherche" class="form-control" placeholder="saisi ici le nom du client pour trouver sa proformat">
                <span id="text"></span>
                <hr>
                <div class="list-group" id="fp_cli">
...
<!-- liste des factures proformat fin -->

<!-- liste des articles du règlement ticket debut -->
<div class="modal fade" tabindex="-1" role="dialog" id="ART_rc">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body" id="art_rc_cli">
...
<script>
    $(document).ready(function () {
        ...
        /*on cache le champ de confirmation pour valider les prix inférieur debu*/
        $(".hide").hide();
        /*on cache le champ de confirmation pour valider les prix inférieur fin*/
        ...
        $('#new_facture_form').on('submit', function(event){
            ...
            $.ajax({
                method: "POST",
                url: "<?php echo base_url('form_facture_pf'); ?>",
                data: $(this).serialize(),
                dataType: "json",
                beforeSend:function(){
                ...
                success: function (data){
                    if(data.error){
                        ...
                        if(data.delais_error != ''){
                            $('#delais_error').html(data.delais_error);
                        }else{
                            $('#delais_error').html('');
                        }
        ...
        $('#op_facture_form').on('submit', function(event){
            ...
            $.ajax({
                method: "POST",
                url: "<?php echo base_url('op_facturepf'); ?>",
                data: $(this).serialize(),
                dataType: "json",
                beforeSend:function(){
        ...
        /**affiche la liste des factures proformat debut */
        
        $(document).on("click", ".fpbtn", function(event){
            event.preventDefault();
            factureproformat_cli(1);
        });
        function factureproformat_cli(page){
            $.ajax({
                method: "GET",
                url: "<?php echo base_url('allproformat/');?>"+page,
                dataType: "json",
                success: function (data) {
                    $("#fp_cli").html(data.infos);
        ...
        $(document).on("click", ".pagination li a", function(event){
            ...
            factureproformat_cli(page);
        });
        /**selectionne tous les articles dans la base des données fin */
        
        /**affiche la liste des factures proformat fin */

        /**affiche la liste des articles d'une  proformat debut */
        $(document).on("click", ".art_fp", function(event){
            var code_rt = $(this).attr('id');
            $.ajax({
                method: "POST",
                url: "<?php echo base_url('all_art_rc_cli');?>",
                data: {code_rt:code_rt},
                dataType: "json",
                success: function (data) {
                    $("#art_rc_cli").html(data.infos);
                    $("#btn").html(data.link);
                    $("#ART_rc").modal('show');
                }
            });
        });
        /**affiche la liste des article d'une  proformat fin */
        ...
        /**rechercher une facture debut*/
         $("#recherche").keyup(function(){
            ...
            if(rechercher !==""){
                $.ajax({
                    method: "POST",
                    url: "<?php echo base_url('allproformat');?>",
        ...
        /**edit document debut */
        $(document).on("click", ".editdocfp", function(event){
            event.preventDefault();
